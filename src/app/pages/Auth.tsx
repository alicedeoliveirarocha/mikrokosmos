      ...
      if (isLogin) {
        ...
        if (result.success) {
          ...
          const errorMsg = result.error?.includes('already registered')
            ...
            : result.error || t('auth.registerError');
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-8">
          <div className="flex gap-2 mb-6">
            <button
              ...
              className={`flex-1 py-3 rounded-xl font-bold transition-all ${isLogin ? 'text-black' : 'text-white/60 bg-white/5'}`}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? t('auth.processing') : isLogin ? (
                <span className="flex items-center justify-center gap-2">
                  <LogIn className="w-5 h-5" />{t('auth.login')}
                </span>
              ) : (
                <span className="flex items-center justify-center gap-2">
                  <UserPlus className="w-5 h-5" />{t('auth.createAccount')}
                </span>
              )}
            </motion.button>
          </form>
