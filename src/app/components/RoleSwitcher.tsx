...
// 🔒 Rede de segurança: emails que SEMPRE podem acessar o switcher,
// mesmo que o papel atual no banco tenha sido trocado para outro role.
// Isso evita a armadilha de ficar "trancado" como cozinha/delivery
// sem conseguir voltar a ser admin.
...
export function RoleSwitcher() {
  ...
  const roles: { value: UserRole; label: string; icon: any; color: string }[] = [
    { value: 'cliente',  label: 'Cliente',  icon: UserCircle, color: '#00FFFF' },
    { value: 'cozinha',  label: 'Cozinha',  icon: ChefHat,    color: '#FF6B35' },
    { value: 'delivery', label: 'Delivery', icon: Bike,       color: '#FFD700' },
              ...
              <div className="bg-black/90 backdrop-blur-xl rounded-2xl border border-white/10 p-6 mx-4">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-sm text-white/60 mt-1">Sistema de Demonstração RBAC</p>
                  </div>
                  <button
                    onClick={() => setIsOpen(false)}
